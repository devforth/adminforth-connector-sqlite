...
import { IAdminForthDataSourceConnector, IAdminForthSingleFilter, IAdminForthAndOrFilter, AdminForthResource, AdminForthResourceColumn, AdminForthConfig, IAggregationRule, IGroupByRule, IGroupByDateTrunc, IGroupByField, AdminForthBaseConnector } from 'adminforth';
...
class SQLiteConnector extends AdminForthBaseConnector implements IAdminForthDataSourceConnector {
  ...
  async getAllTables(): Promise<Array<string>> {
    ...
    const rows = stmt.all() as AnyRow[];
    return rows.map((row: AnyRow) => row.name);
  }
  async getAllColumnsInTable(tableName: string): Promise<Array<{ name: string; sampleValue?: any }>> {
    ...
    const columns = stmt.all() as AnyRow[];
  
    const orderByField = columns.find((c: AnyRow) => ['created_at', 'id'].includes(c.name))?.name;
  
    let sampleRow: AnyRow = {};
    if (orderByField) {
    ...
    async hasSQLiteCascadeFk(resource: AdminForthResource, config: AdminForthConfig): Promise<boolean> {
      ...
      const fkRows = await fkStmt.all() as AnyRow[];
      const fkMap: { [colName: string]: boolean } = {};
      fkRows.forEach((fk: AnyRow) => { fkMap[fk.from] = fk.on_delete?.toUpperCase() === 'CASCADE'; });
    ...
    async discoverFields(resource: AdminForthResource, config: AdminForthConfig): Promise<{[key: string]: AdminForthResourceColumn}> {
        ...
        const stmt = this.client.prepare(`PRAGMA table_info(${tableName})`);
        const rows = await stmt.all() as AnyRow[];       
    ...
    setFieldValue(field: AdminForthResourceColumn, value: any): any {
      if (field.type == AdminForthDataTypes.DATETIME) {
        ...
        // SQLite does not have a native boolean type, so we store booleans as integers
    ...
    getFilterString(filter: IAdminForthSingleFilter | IAdminForthAndOrFilter): string {
      if ((filter as IAdminForthSingleFilter).field) {
        ...
        let operator = this.OperatorsMap[filter.operator as keyof typeof this.OperatorsMap];
      ...
      return (filter as IAdminForthAndOrFilter).subFilters.map((f) => {
        ...
        return `(${this.getFilterString(f) })`;
      }).join(` ${this.OperatorsMap[filter.operator as keyof typeof this.OperatorsMap]} `);
    }
    ...
    async getDataWithOriginalTypes({ resource, limit, offset, sort, filters, columns }: {
      ...
      const selectedColumns = (columns ?? resource.dataSourceColumns).map((col: AdminForthResourceColumn) => col.name).join(', ');
      const tableName = resource.table;
    ...
    async getCount({ resource, filters }: { resource: AdminForthResource, filters?: IAdminForthAndOrFilter }) {
      ...
      const tableName = resource.table;
      const where = this.whereClause(normalizedFilters as IAdminForthAndOrFilter);
      const filterValues = this.getFilterParams(normalizedFilters as IAdminForthAndOrFilter);
